...
class SlotTaggingPredictor(Predictor):
    def predict(self, inputs: List[JsonDict]) -> JsonDict:
        ...
        # print(output_dict[0]['tag_logits'])   
        # print(output_dict[0]['predicted_tags'])
...
def stat_slot_nums(data_path, mode="all"):
    ...
    Args:
        path: 目录路径，如/mnt/2/ljc/workspace/self-training-for-slot-filling/results/snips/by_count_10/bert_self_train_debug_3/saved_data/epoch_1/labeled.
...
def calculate_slot_weight(labeled_slots_counter, mu=1, pattern_id=1, slot_label_list=None, save_data_path=None):
    ...
    for slot, num in labeled_slots_counter.items():
        ...
        prob = (nums_sort_list[L-num_rank]/N_1) ** mu                    # 公式中的 mu
...
def calculate_sent_confidence(slot_weight_dict, predict_labels, max_score, gama=2):
    """ 计算句子置信度 [0417]
    ...
    Args:
        ...
        gama: int型超参. 特例："infinity"表示当gama为无穷大时, 相当于python当中的float("int").
    ...
    for i, p in enumerate(predict_labels):    # 返回的预测标签p直接是标签,如I-artist
        if p.startswith('B') or p.startswith('I'):
            ...
            elif isinstance(gama, int):
                score_sum += (1 + gama * slot_weight_dict[p[2:]]) * max_score[i]
                weight_sum += (1 + gama * slot_weight_dict[p[2:]])
...
if __name__ == "__main__":
    # labeled_slots_counter = stat_slot_nums(data_path="/mnt/2/ljc/workspace/self-training-for-slot-filling/results/snips/by_count_10/bert_self_train_debug_2/saved_data/epoch_2/labeled", mode="add")
    # print(calculate_slot_weight(labeled_slots_counter, save_data_path="/mnt/3/prisNlpWorkSpace/self-training-for-slot-filling"))
