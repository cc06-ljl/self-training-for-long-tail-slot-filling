...
from allennlp.modules.token_embedders import TokenCharactersEncoder, PretrainedTransformerMismatchedEmbedder, PretrainedTransformerEmbedder
...
from losses.LDAMCRF import LDAMLogits
from losses.OursLDAM import LDAMACSLoss
...
class SlotTaggingModel(Model):
    def __init__(self, 
        ...
        self.use_ldam_acsl = use_ldam_acsl
        self.ldamloss_weight = lambda1
        
        if use_ldam:
            if cls_num_list:
                self.ldamloss = LDAMLoss(cls_num_list, max_m=max_m, weight=slot_weight, s=s)   # 累计数据 类别数量
                # self.ldamloss = LDAMLoss(cls_num_list, max_m=0.5, weight=slot_weight, s=1)    # use_margin_crf为True时，s要设置得小些，不然无法收敛
            # else:
            #     raise Exception("LDAMLoss initialization failed!")
        elif use_ldam_acsl:
            if cls_num_list:
                self.ldamloss = LDAMACSLoss(cls_num_list, max_m=max_m, weight=slot_weight, s=s, acsl_thre=acsl_threshold, lambda_ = lambda2)
    ...
    def _inner_forward(self,
        ...
        if self.use_crf:
            ...
            # print('____________________________')
            # print(torch.softmax(tag_logits,dim=1))
            # print('____________________________')
            # exit()
        else:
        ...
        if labels is not None:
            # --- TODO: 添加LDAM Loss【Done: 0315】
            output["loss"] = 0
            if self.use_ldam or self.use_ldam_acsl:
                # 对tag_logits计算一层LDAM损失
                ldam_loss = self.ldamloss(tag_logits.view(-1, tag_logits.shape[-1]), labels.view(-1))

                output["loss"] += self.ldamloss_weight * ldam_loss

            # Note: ldam_loss 比log_likelihood小很多，16.69 v.s. 2842; 不知道随着训练会如何进行
            # --------------
